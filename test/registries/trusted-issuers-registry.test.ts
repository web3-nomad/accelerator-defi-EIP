    ...
    describe('when sender is the owner', () => {
      ...
      describe('when issuer is already registered', () => {
        it('should revert', async () => {
          ...
          const claimTopicsParsed = claimTopics.map(a => a); // hackish solution to TypeError: Cannot assign to read only property '0' of object '[object Array]'

          await expect(
            trustedIssuersRegistry.connect(deployer).addTrustedIssuer(await claimIssuerContract.getAddress(), claimTopicsParsed)
    ...
    describe('when sender is the owner', () => {
      ...
      describe('when claim topics array have more that 15 elements', () => {
        it('should revert', async () => {
          ...
          await expect(trustedIssuersRegistry.connect(deployer).updateIssuerClaimTopics(await claimIssuerContract.getAddress(), claimTopics)).to.be.revertedWith(
            'cannot have more than 15 claim topics',
          );
        });
      });
      ...
      describe('when claim topics array is empty', () => {
        it('should revert', async () => {
          ...
          await expect(trustedIssuersRegistry.connect(deployer).updateIssuerClaimTopics(await claimIssuerContract.getAddress(), [])).to.be.revertedWith(
      ...
      describe('when issuer is registered', () => {
        it('should update the topics of the trusted issuers', async () => {
          ...
          const tx = await trustedIssuersRegistry.connect(deployer).updateIssuerClaimTopics(await claimIssuerContract.getAddress(), [66, 100]);
          await expect(tx).to.emit(trustedIssuersRegistry, 'ClaimTopicsUpdated').withArgs(await claimIssuerContract.getAddress(), [66, 100]);

          await expect(trustedIssuersRegistry.hasClaimTopic(await claimIssuerContract.getAddress(), 66)).to.eventually.be.true;
          await expect(trustedIssuersRegistry.hasClaimTopic(await claimIssuerContract.getAddress(), 100)).to.eventually.be.true;
          await expect(trustedIssuersRegistry.hasClaimTopic(await claimIssuerContract.getAddress(), claimTopics[0])).to.eventually.be.false;
          await expect(trustedIssuersRegistry.getTrustedIssuerClaimTopics(await claimIssuerContract.getAddress())).to.eventually.deep.eq([66, 100]);
        });
      });
    });
  });
