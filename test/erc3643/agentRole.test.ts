  ...
  describe('.addAgent', () => {
    describe('when the sender is not the owner', () => {
      it('should reverts', async () => {
        ...
        await expect(agentRole.connect(bobWallet).addAgent(aliceWallet.address))
          .to.be.revertedWithCustomError(agentRole, "OwnableUnauthorizedAccount");
      });
    });
    ...
    describe('when the sender is the owner', () => {
      describe('when address to add is the zero address', () => {
        it('should reverts', async () => {
          ...
          await expect(agentRole.connect(ownerWallet).addAgent(ethers.ZeroAddress)).to.be.revertedWith('invalid argument - zero address');
        });
      });
  ...
  describe('.removeAgent', () => {
    describe('when the sender is not the owner', () => {
      it('should reverts', async () => {
        ...
        await expect(agentRole.connect(bobWallet).removeAgent(aliceWallet.address)).to.be.revertedWithCustomError(agentRole, "OwnableUnauthorizedAccount");
      });
    });
    ...
    describe('when the sender is the owner', () => {
      describe('when address to add is the zero address', () => {
        it('should reverts', async () => {
          ...
          await expect(agentRole.connect(ownerWallet).removeAgent(ethers.ZeroAddress)).to.be.revertedWith(
