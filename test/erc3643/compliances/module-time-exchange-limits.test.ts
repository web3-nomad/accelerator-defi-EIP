...
describe('Compliance Module: TimeExchangeLimits', () => {
        ...
        describe('when limit array size not exceeded', () => {
          it('should add new limit', async () => {
            ...
            await expect(tx)
              ...
              .withArgs(await context.contracts.compliance.getAddress(), exchangeID, 100, 1);

            const limits = await context.contracts.complianceModule.getExchangeLimits(await context.suite.compliance.getAddress(), exchangeID);
  ...
  describe('.getExchangeCounter', () => {
    it('should return counter', async () => {
      ...
      const counter = await context.suite.complianceModule.getExchangeCounter(await context.suite.compliance.getAddress(), exchangeID, investorID, 10000);
  ...
  describe('.removeExchangeID', () => {
    describe('when calling directly', () => {
      it('should revert', async () => {
        ...
        await expect(context.contracts.complianceModule.connect(context.accounts.bobWallet).removeExchangeID(exchangeID)).to.be.revertedWithCustomError(context.contracts.complianceModule, 'OwnableUnauthorizedAccount')
      });
    });
      ...
      describe('when receiver is not an exchange', () => {
        describe('when sender is not a token agent', () => {
          it('should not set limits', async () => {
            ...
            const counter = await context.suite.complianceModule.getExchangeCounter(await context.suite.compliance.getAddress(), receiverID, investorID, 10000);
            expect(counter.timer).to.be.eq(0);
            expect(counter.value).to.be.eq(0);
          });
        });
