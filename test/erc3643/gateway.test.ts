    ...
    describe('when called by owner', () => {
      describe('if required parameters are not filled', () => {
        it('should revert', async () => {
          ...
          await expect(gateway.setDeploymentFee(100, await context.suite.token.getAddress(), ethers.ZeroAddress)).to.be.revertedWithCustomError(
            gateway,
            'ZeroAddress',
          );
        });
      ...
      describe('if called properly', () => {
        it('should set new fees structure', async () => {
          ...
          const tx = await gateway.setDeploymentFee(100, await context.suite.token.getAddress(), context.accounts.deployer.address);
    ...
    describe('when called by owner', () => {
      ...
      describe('if discount valid', () => {
        it('should apply discount', async () => {
          ...
          await gateway.setDeploymentFee(20000, await context.suite.token.getAddress(), context.accounts.deployer.address);
    ...
    describe('when called by agent', () => {
      ...
      describe('if all discounts are valid', () => {
        it('should apply discounts to all deployers', async () => {
          ...
          await gateway.setDeploymentFee(deploymentFee, await context.suite.token.getAddress(), context.accounts.deployer.address);
      ...
      describe('when public deployments are enabled', () => {
        describe('when try to deploy on behalf', () => {
          it('should revert', async () => {
            ...
            const gateway = await ethers.deployContract('TREXGateway', [await context.factories.trexFactory.getAddress(), true], context.accounts.deployer);
            await context.factories.trexFactory.transferOwnership(await gateway.getAddress());
        ...
        describe('when deployment fees are activated', () => {
          describe('when caller has no discount', () => {
            it('should deploy a token for full fee', async () => {
              ...
              await gateway.setDeploymentFee(20000, await feeToken.getAddress(), context.accounts.deployer.address);
              await gateway.enableDeploymentFee(true);

              await feeToken.connect(context.accounts.anotherWallet).approve(await gateway.getAddress(), 20000);
              const tx = await gateway.connect(context.accounts.anotherWallet).deployTREXSuite(
                {
                  owner: context.accounts.anotherWallet.address,
                  name: 'Token name',
        ...
        describe('when deployment fees are activated', () => {
          describe('when caller has no discount', () => {
            it('should deploy tokens for full fee in a batch', async () => {
              ...
              await feeToken.connect(context.accounts.anotherWallet).approve(await gateway.getAddress(), 100000);
          ...
          describe('when caller has 50% discount', () => {
            it('should deploy tokens for half fee in a batch', async () => {
              ...
              await gateway.enableDeploymentFee(true);
              await gateway.applyFeeDiscount(context.accounts.anotherWallet.address, 5000);
              await feeToken.connect(context.accounts.anotherWallet).approve(await gateway.getAddress(), 50000);
