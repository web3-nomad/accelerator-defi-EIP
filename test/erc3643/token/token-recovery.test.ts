  ...
  describe('.recoveryAddress()', () => {
    describe('when sender is not an agent', () => {
      it('should reverts', async () => {
        ...
        await expect(token.connect(anotherWallet).recoveryAddress(bobWallet.address, anotherWallet.address, await bobIdentity.getAddress())).to.be.revertedWith(
    ...
    describe('when sender is an agent', () => {
      describe('when wallet to recover has no balance', () => {
        it('should revert', async () => {
          ...
          await expect(token.connect(tokenAgent).recoveryAddress(bobWallet.address, anotherWallet.address, await bobIdentity.getAddress())).to.be.revertedWith(
      ...
      describe('when wallet is frozen', () => {
        it('should recover and freeze the new wallet', async () => {
          ...
          const tx = await token.connect(tokenAgent).recoveryAddress(bobWallet.address, anotherWallet.address, await bobIdentity.getAddress());
          await expect(token.isFrozen(anotherWallet.address)).to.be.eventually.true;
          await expect(tx).to.emit(token, 'RecoverySuccess').withArgs(bobWallet.address, anotherWallet.address, await bobIdentity.getAddress());
