  ...
  describe('.setOnchainID()', () => {
    describe('when the caller is not the owner', () => {
      it('should revert', async () => {
        ...
        await expect(token.connect(anotherWallet).setOnchainID(ethers.ZeroAddress)).to.be.revertedWithCustomError(token, 'OwnableUnauthorizedAccount')
      });
    });
  ...
  describe('.setIdentityRegistry()', () => {
    describe('when the caller is not the owner', () => {
      it('should revert', async () => {
        ...
        await expect(token.connect(anotherWallet).setIdentityRegistry(ethers.ZeroAddress)).to.be.revertedWithCustomError(token, 'OwnableUnauthorizedAccount')
      });
    });
  });
  ...
  describe('.setCompliance', () => {
    describe('when the caller is not the owner', () => {
      it('should revert', async () => {
        ...
        await expect(token.connect(anotherWallet).setCompliance(ethers.ZeroAddress)).to.be.revertedWithCustomError(token, 'OwnableUnauthorizedAccount')
      });
    });
  });
