...
describe('DVATransferManager', () => {
  ...
  async function deployFullSuiteWithVerifiedTransferManager() {
    ...
    await context.suite.identityRegistry.connect(context.accounts.tokenAgent).registerIdentity(await context.suite.transferManager.getAddress(), identity, 0);
    return context;
  }
  ...
  async function deployFullSuiteWithNonSequentialTransfer() {
    ...
    await context.suite.transferManager
      ...
      .setApprovalCriteria(await context.suite.token.getAddress(), true, true, false, [context.accounts.charlieWallet.address]);

    await context.suite.token.connect(context.accounts.aliceWallet).approve(await context.suite.transferManager.getAddress(), 100000);
    ...
    await context.suite.transferManager
      ...
      .initiateTransfer(await context.suite.token.getAddress(), context.accounts.bobWallet.address, 100);
  ...
  async function deployFullSuiteWithSequentialTransfer() {
    ...
    await context.suite.transferManager
      .connect(context.accounts.tokenAgent)
      .setApprovalCriteria(await context.suite.token.getAddress(), true, true, true, [context.accounts.charlieWallet.address]);
      ...
      it('should revert', async () => {
        ...
        await expect(
          context.suite.transferManager
            ...
            .setApprovalCriteria(await context.suite.token.getAddress(), false, true, true, []),
      ...
      describe('when DVA Manager is verified for the token', () => {
        describe('when token is not already registered', () => {
          it('should modify approval criteria', async () => {
            ...
            await context.suite.identityRegistry
              .connect(context.accounts.tokenAgent)
              .registerIdentity(await context.suite.transferManager.getAddress(), identity, 0);

            const tx = await context.suite.transferManager
              .connect(context.accounts.tokenAgent)
              .setApprovalCriteria(await context.suite.token.getAddress(), true, true, true, [
            ...
            const approvalCriteria = await context.suite.transferManager.getApprovalCriteria(await context.suite.token.getAddress());
        ...
        describe('when token is already registered', () => {
          it('should modify approval criteria', async () => {
            ...
            const previousApprovalCriteria = await context.suite.transferManager.getApprovalCriteria(await context.suite.token.getAddress());

            const tx = await context.suite.transferManager
              .connect(context.accounts.tokenAgent)
              .setApprovalCriteria(await context.suite.token.getAddress(), false, false, false, [context.accounts.davidWallet.address]);
            ...
            await expect(tx)
              .to.emit(context.suite.transferManager, 'ApprovalCriteriaSet')
              .withArgs(await context.suite.token.getAddress(), false, false, false, [context.accounts.davidWallet.address], approvalCriteria.hash);
          });
        });
      });
    });
    ...
    describe('when token is not registered to the DVA manager', () => {
      it('should revert', async () => {
        ...
        await expect(
          context.suite.transferManager
            .connect(context.accounts.aliceWallet)
            .initiateTransfer(await context.suite.token.getAddress(), context.accounts.bobWallet.address, 10),
      ...
      describe('when recipient is not verified for the token', () => {
        it('should revert', async () => {
          ...
          await expect(
            context.suite.transferManager
              .connect(context.accounts.aliceWallet)
              .initiateTransfer(await context.suite.token.getAddress(), context.accounts.anotherWallet.address, 10),
      ...
      describe('when amount is higher than sender balance', () => {
        it('should revert', async () => {
          ...
          await expect(
            context.suite.transferManager
              .connect(context.accounts.aliceWallet)
              .initiateTransfer(await context.suite.token.getAddress(), context.accounts.bobWallet.address, 100000),
      ...
      describe('when sender has enough balance', () => {
        describe('when includeRecipientApprover is true', () => {
          it('should initiate the transfer with recipient approver', async () => {
            ...
            await context.suite.transferManager
              .connect(context.accounts.tokenAgent)
              .setApprovalCriteria(await context.suite.token.getAddress(), true, false, true, []);
            ...
            await expect(tx)
              ...
              .withArgs(
                ...
                100,
                (
                  await context.suite.transferManager.getApprovalCriteria(await context.suite.token.getAddress())
                ).hash,
              );
        ...
        describe('when additional approvers exist', () => {
          it('should initiate the transfer with token agent approver', async () => {
            ...
            await context.suite.transferManager
              .connect(context.accounts.tokenAgent)
              .setApprovalCriteria(await context.suite.token.getAddress(), false, false, true, [
                context.accounts.charlieWallet.address,
                context.accounts.anotherWallet.address,
              ]);
        ...
        describe('when all criteria are enabled', () => {
          it('should initiate the transfer with all approvers', async () => {
            ...
            const dvaBalance = await context.suite.token.balanceOf(await context.suite.transferManager.getAddress());
      ...
      describe('when trying to approve before approval state reset', () => {
        it('should reset approvers', async () => {
          ...
          await expect(tx)
            ...
            .withArgs(context.transferID, (await context.suite.transferManager.getApprovalCriteria(await context.suite.token.getAddress())).hash);
      ...
      describe('when all parties approve the transfer', () => {
        it('should complete', async () => {
          ...
          await expect(tx)
            ...
            .withArgs(context.transferID, await context.suite.token.getAddress(), context.accounts.aliceWallet.address, context.accounts.bobWallet.address, 100);
  ...
  describe('.delegateApproveTransfer', () => {
    ...
    describe('when transfer does not exist', () => {
      it('should revert', async () => {
        ...
        await expect(
          context.suite.transferManager.delegateApproveTransfer(transferID, [await signTransfer(transferID, context.accounts.charlieWallet as any)]),
      ...
      it('should revert', async () => {
        ...
        await expect(
          context.suite.transferManager.delegateApproveTransfer(context.transferID, [
            await signTransfer(context.transferID, context.accounts.charlieWallet as any),
          ]),
    ...
    describe('when approval criteria are changed after the transfer has been initiated', () => {
      describe('when trying to approve before approval state reset', () => {
        it('should reset approvers', async () => {
          ...
          const tx = context.suite.transferManager
            .connect(context.accounts.anotherWallet)
            .delegateApproveTransfer(context.transferID, [await signTransfer(context.transferID, context.accounts.charlieWallet as any)]);
      ...
      describe('when trying to approve after approval state reset', () => {
        it('should approve', async () => {
          ...
          const tx = context.suite.transferManager
            .connect(context.accounts.anotherWallet)
            .delegateApproveTransfer(context.transferID, [await signTransfer(context.transferID, context.accounts.davidWallet as any)]);
          await expect(tx)
            .to.emit(context.suite.transferManager, 'TransferApproved')
            .withArgs(context.transferID, context.accounts.davidWallet.address);
        });
    ...
    describe('when sequential approval is disabled', () => {
      describe('when caller is not an approver', () => {
        it('should revert', async () => {
          ...
          await expect(
            context.suite.transferManager.delegateApproveTransfer(context.transferID, [
              await signTransfer(context.transferID, context.accounts.anotherWallet as any),
            ]),
        ...
        it('should complete', async () => {
          ...
          const tx = await context.suite.transferManager
            ...
            .delegateApproveTransfer(context.transferID, [
              await signTransfer(context.transferID, context.accounts.tokenAgent as any),
              await signTransfer(context.transferID, context.accounts.bobWallet as any),
              await signTransfer(context.transferID, context.accounts.charlieWallet as any),
            ]);
  ...
  describe('.getTransfer', () => {
    ...
    describe('when transfer exists', () => {
      it('should return transfer', async () => {
        ...
        const transfer = await context.suite.transferManager.getTransfer(context.transferID);
        expect(transfer.tokenAddress).to.be.eq(await context.suite.token.getAddress());
  ...
  describe('.getApprovalCriteria', () => {
    describe('when token is not registered', () => {
      it('should revert', async () => {
        const context = await loadFixture(deployFullSuiteWithVerifiedTransferManager);
        await expect(context.suite.transferManager.getApprovalCriteria(await context.suite.token.getAddress())).to.be.revertedWithCustomError(
          context.suite.transferManager,
