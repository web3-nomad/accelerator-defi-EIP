  ...
  describe('.updateIdentity()', () => {
    describe('when sender is not an agent', () => {
      it('should revert', async () => {
        ...
        await expect(identityRegistry.connect(anotherWallet).updateIdentity(await bobIdentity.getAddress(), await charlieIdentity.getAddress())).to.be.revertedWith(
  ...
  describe('.updateCountry()', () => {
    describe('when sender is not an agent', () => {
      it('should revert', async () => {
        ...
        await expect(identityRegistry.connect(anotherWallet).updateCountry(await bobIdentity.getAddress(), 100)).to.be.revertedWith(
          'AgentRole: caller does not have the Agent role',
        );
      });
    });
  ...
  describe('.isVerified()', () => {
    describe('when there are no require claim topics', () => {
      it('should return true when the identity is registered', async () => {
        ...
        await identityRegistry.connect(tokenAgent).registerIdentity(charlieWallet.address, await charlieIdentity.getAddress(), 0);
    ...
    describe('when the claim issuer throws an error', () => {
      it('should return true if there is another valid claim', async () => {
        ...
        const claimTopics = (await claimTopicsRegistry.getClaimTopics()).map(c => c);
        await trustedIssuersRegistry.removeTrustedIssuer(await claimIssuerContract.getAddress());
        await trustedIssuersRegistry.addTrustedIssuer(await trickyClaimIssuer.getAddress(), claimTopics);
        await trustedIssuersRegistry.addTrustedIssuer(await claimIssuerContract.getAddress(), claimTopics);
        ...
        await aliceIdentity.connect(aliceWallet).addClaim(claimTopics[0], 1, await trickyClaimIssuer.getAddress(), '0x00', '0x00', '');
