...
describe('DVDTransferManager', () => {
  ...
  async function deployFullSuiteWithTransferManagerAndInitiatedTransfer() {
    ...
    await context.suite.erc20A.connect(context.accounts.aliceWallet).approve(await context.suite.transferManager.getAddress(), 1000);
    await context.suite.erc20B.mint(context.accounts.bobWallet.address, 500);
    await context.suite.erc20B.connect(context.accounts.bobWallet).approve(await context.suite.transferManager.getAddress(), 500);
    const tx = await context.suite.transferManager
      .connect(context.accounts.aliceWallet)
      .initiateDVDTransfer(await context.suite.erc20A.getAddress(), 1000, context.accounts.bobWallet.address, await context.suite.erc20B.getAddress(), 500);

    await tx.wait();

    const event = (await context.suite.transferManager.queryFilter(context.suite.transferManager.filters.DVDTransferInitiated, -1))[0];
  ...
  describe('.modifyFee()', () => {
    describe('when sender is neither the DVD contract owner neither a TREX token owner', () => {
      it('should revert', async () => {
        ...
        await expect(
          transferManager.connect(anotherWallet).modifyFee(await erc20A.getAddress(), await erc20B.getAddress(), 1, 1, 1, charlieWallet.address, davidWallet.address),
    ...
    describe('when sender is a TREX token owner', () => {
      it('should revert for invalid fee settings', async () => {
        ...
        await expect(
          transferManager.connect(deployer).modifyFee(await token.getAddress(), await erc20A.getAddress(), 1, 1, 1, charlieWallet.address, davidWallet.address),
      ...
      it('should revert for invalid fee settings', async () => {
        ...
        await expect(
          transferManager.connect(deployer).modifyFee(await erc20A.getAddress(), await token.getAddress(), 1, 1, 1, charlieWallet.address, davidWallet.address),
        ).to.be.revertedWith('invalid fee settings');
      });
    });
      ...
      describe('when fee settings are not valid', () => {
        describe('when fee1 is above max for feeBase', () => {
          it('should revert', async () => {
            ...
            await expect(
              transferManager.connect(deployer).modifyFee(await erc20A.getAddress(), await erc20B.getAddress(), 1000, 1, 2, charlieWallet.address, davidWallet.address),
            ).to.be.revertedWith('invalid fee settings');
          });
        });
        ...
        describe('when fee2 is above max for feeBase', () => {
          it('should revert', async () => {
            ...
            await expect(
              transferManager.connect(deployer).modifyFee(await erc20A.getAddress(), await erc20B.getAddress(), 1, 1000, 2, charlieWallet.address, davidWallet.address),
            ).to.be.revertedWith('invalid fee settings');
          });
        });
        ...
        describe('when feeBase is less than 2', () => {
          it('should revert', async () => {
            ...
            await expect(
              transferManager.connect(deployer).modifyFee(await erc20A.getAddress(), await erc20B.getAddress(), 0, 0, 1, charlieWallet.address, davidWallet.address),
            ).to.be.revertedWith('invalid fee settings');
          });
        });
        ...
        describe('when feeBase is more than 5', () => {
          it('should revert', async () => {
            ...
            await expect(
              transferManager.connect(deployer).modifyFee(await erc20A.getAddress(), await erc20B.getAddress(), 1000, 1, 10, charlieWallet.address, davidWallet.address),
            ).to.be.revertedWith('invalid fee settings');
          });
        });
        ...
        describe('when fee1 is set, then fee1Wallet cannot be zero address', () => {
          it('should revert', async () => {
            ...
            await expect(
              transferManager.connect(deployer).modifyFee(await erc20A.getAddress(), await erc20B.getAddress(), 2, 0, 2, ethers.ZeroAddress, davidWallet.address),
        ...
        describe('when fee2 is set, then fee2Wallet cannot be zero address', () => {
          it('should revert', async () => {
            ...
            await expect(
              transferManager
                .connect(deployer)
                .modifyFee(await erc20A.getAddress(), await erc20B.getAddress(), 0, 1, 2, ethers.ZeroAddress, ethers.ZeroAddress),
      ...
      describe('when fee settings are valid', () => {
        it('should set fee settings', async () => {
          ...
          const tx = await transferManager
            .connect(deployer)
            .modifyFee(await erc20A.getAddress(), await erc20B.getAddress(), 2, 1, 2, charlieWallet.address, davidWallet.address);
          await expect(tx)
            .to.emit(transferManager, 'FeeModified')
            .withArgs(
              ethers.keccak256(AbiCoder.defaultAbiCoder().encode(['address', 'address'], [await erc20A.getAddress(), await erc20B.getAddress()])),
          ...
          await expect(tx)
            .to.emit(transferManager, 'FeeModified')
            .withArgs(
              ethers.keccak256(AbiCoder.defaultAbiCoder().encode(['address', 'address'], [await erc20B.getAddress(), await erc20A.getAddress()])),
              await erc20B.getAddress(),
              await erc20A.getAddress(),
              1,
              2,
        ...
        it('should set fee settings', async () => {
          ...
          const tx = await transferManager
            .connect(deployer)
            .modifyFee(await erc20A.getAddress(), await erc20B.getAddress(), 2, 0, 2, charlieWallet.address, ethers.ZeroAddress);
          await expect(tx)
            .to.emit(transferManager, 'FeeModified')
            .withArgs(
              ethers.keccak256(AbiCoder.defaultAbiCoder().encode(['address', 'address'], [await erc20A.getAddress(), await erc20B.getAddress()])),
  ...
  describe('.initiateDVDTransfer()', () => {
    describe('when sender has not enough balance to initiate the transfer', () => {
      it('should revert', async () => {
        ...
        await expect(
          transferManager.connect(charlieWallet).initiateDVDTransfer(await erc20A.getAddress(), 1000, charlieWallet.address, await erc20B.getAddress(), 1000),
    ...
    describe('when sender has not setup a sufficient allowance', () => {
      it('should revert', async () => {
        ...
        await expect(
          transferManager.connect(charlieWallet).initiateDVDTransfer(await erc20A.getAddress(), 1000, davidWallet.address, await erc20B.getAddress(), 1000),
    ...
    describe('when counterpart is zero address', () => {
      it('should revert', async () => {
        ...
        await erc20A.connect(deployer).mint(charlieWallet.address, 1000);
        await erc20A.connect(charlieWallet).approve(await transferManager.getAddress(), 1000);
        await expect(
          transferManager.connect(charlieWallet).initiateDVDTransfer(await erc20A.getAddress(), 1000, ethers.ZeroAddress, await erc20B.getAddress(), 1000),
    ...
    describe('when token2 has no supply', () => {
      it('should revert', async () => {
        ...
        await erc20A.connect(charlieWallet).approve(await transferManager.getAddress(), 1000);
        await expect(
          transferManager.connect(charlieWallet).initiateDVDTransfer(await erc20A.getAddress(), 1000, davidWallet.address, await erc20C.getAddress(), 1000),
    ...
    describe('when transfer condition are met', () => {
      it('should store an initiated transfer', async () => {
        ...
        await erc20A.connect(deployer).mint(charlieWallet.address, 1000);
        await erc20A.connect(charlieWallet).approve(await transferManager.getAddress(), 1000);
        const tx = await transferManager.connect(charlieWallet).initiateDVDTransfer(await erc20A.getAddress(), 1000, davidWallet.address, await erc20B.getAddress(), 500);
        await expect(tx)
          .to.emit(transferManager, 'DVDTransferInitiated')
          .withArgs(anyValue, charlieWallet.address, await erc20A.getAddress(), 1000, davidWallet.address, await erc20B.getAddress(), 500);
      });
    });
  });
    ...
    describe('when transfer was initiated', () => {
      ...
      describe('when sender is the counterpart and transfer has fees', () => {
        it('should execute the transfer with fees', async () => {
          ...
          await transferManager.modifyFee(await erc20A.getAddress(), await erc20B.getAddress(), 1, 2, 2, charlieWallet.address, davidWallet.address);
  ...
  describe('.isTREXAgent()', () => {
    describe('when address is not a TREX agent', () => {
      it('should return false', async () => {
        ...
        expect(await transferManager.isTREXAgent(await token.getAddress(), anotherWallet.address)).to.be.false;
      });
    });
    ...
    describe('when address is a TREX agent', () => {
      it('should return true', async () => {
        ...
        expect(await transferManager.isTREXAgent(await token.getAddress(), tokenAgent.address)).to.be.true;
      });
    });
  });
  ...
  describe('.isTREXOwner()', () => {
    describe('when address is not a TREX owner', () => {
      it('should return false', async () => {
        ...
        expect(await transferManager.isTREXOwner(await token.getAddress(), anotherWallet.address)).to.be.false;
      });
    });
    ...
    describe('when address is a TREX agent', () => {
      it('should return true', async () => {
        ...
        expect(await transferManager.isTREXOwner(await token.getAddress(), deployer.address)).to.be.true;
      });
    });
  });
  ...
  describe('.isTREXToken()', () => {
    describe('When token does not have an Identity Registry (probably not a ERC3643 token', () => {
      it('should return false', async () => {
        ...
        expect(await transferManager.isTREX(await token.getAddress())).to.be.false;
      });
    });
  });
});
