...
describe('Compliance Module: ExchangeMonthlyLimits', () => {
    ...
    describe('when calling via compliance', () => {
      it('should update the limit', async () => {
        ...
        await expect(tx)
          ...
          .withArgs(await context.contracts.compliance.getAddress(), exchangeID, 100);
      });
    });
  });
  ...
  describe('.getExchangeMonthlyLimit', () => {
    it('should return monthly limit', async () => {
      ...
      expect(await context.contracts.complianceModule.getExchangeMonthlyLimit(await context.suite.compliance.getAddress(), exchangeID)).to.be.eq(100);
    });
  });
        ...
        describe('when sender is not a token agent', () => {
          describe('when the exchange monthly limit is not exceeded', () => {
            it('should increase exchange counter', async () => {
              ...
              const counter = await context.suite.complianceModule.getMonthlyCounter(await context.suite.compliance.getAddress(), exchangeID, investorID);
          ...
          describe('when the exchange month is finished', () => {
            it('should set monthly timer', async () => {
              ...
              const timer = await context.suite.complianceModule.getMonthlyTimer(await context.suite.compliance.getAddress(), exchangeID, investorID);
          ...
          describe('when the exchange month is not finished', () => {
            it('should not update monthly timer', async () => {
              ...
              const previousTimer = await context.suite.complianceModule.getMonthlyTimer(await context.suite.compliance.getAddress(), exchangeID, investorID);
              await context.suite.compliance.callModuleFunction(
                new ethers.Interface(['function moduleTransferAction(address _from, address _to, uint256 _value)']).encodeFunctionData(
                  'moduleTransferAction',
                  [from, to, 11],
      ...
      describe('when receiver is not an exchange', () => {
        describe('when sender is not a token agent', () => {
          it('should not set limits', async () => {
            ...
            const counter = await context.suite.complianceModule.getMonthlyCounter(await context.suite.compliance.getAddress(), receiverID, investorID);
            expect(counter).to.be.eq(0);

            const timer = await context.suite.complianceModule.getMonthlyTimer(await context.suite.compliance.getAddress(), receiverID, investorID);
            expect(timer).to.be.eq(0);
          });
        });
    ...
    describe('when receiver is exchange', () => {
      ...
      describe('when monthly counter does not exceed the monthly limit', () => {
        it('should return true', async () => {
          ...
          expect(await context.suite.complianceModule.moduleCheck(from, to, 40, await context.suite.compliance.getAddress())).to.be.true;
        });
      });
    });
  });
