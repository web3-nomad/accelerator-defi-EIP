...
library SafeHTS {
    ...
    function safeCreateNonFungibleToken(
        IHederaTokenService.HederaToken memory token
    ) internal nonEmptyExpiry(token) returns (address tokenAddress) {
        int responseCode;
        (bool success, bytes memory result) = precompileAddress.call{value: msg.value}(
    ...
    function safeGetTokenCustomFees(
        address token
    )
        internal
        returns (
    ...
    function safeGetTokenKey(address token, uint keyType) internal returns (IHederaTokenService.KeyValue memory key) {
        ...
        (responseCode, key) = success
            ...
            : (HederaResponseCodes.UNKNOWN, key);
        require(responseCode == HederaResponseCodes.SUCCESS, "Safe get token key failed!");
    }

    function safeGetNonFungibleTokenInfo(
        address token,
        int64 serialNumber
    ) internal returns (IHederaTokenService.NonFungibleTokenInfo memory nonFungibleTokenInfo) {
        int32 responseCode;
        (bool success, bytes memory result) = precompileAddress.call(
    ...
    function safeUpdateTokenInfo(
        address token,
        IHederaTokenService.HederaToken memory tokenInfo
    ) internal nonEmptyExpiry(tokenInfo) {
        int32 responseCode;
        (bool success, bytes memory result) = precompileAddress.call(
    ...
    function safeGetTokenType(address token) internal returns (int32 tokenType) {
        ...
        (responseCode, tokenType) = success
            ...
            : (HederaResponseCodes.UNKNOWN, int32(0));
